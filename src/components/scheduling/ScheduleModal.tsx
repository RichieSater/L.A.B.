...
export function ScheduleModal({ advisorId, onClose }: ScheduleModalProps) {
  ...
  const { scheduleSession } = useScheduling();
  ...
  async function handleSchedule() {
    ...
    await scheduleSession(advisorId, scheduledAt);
    setSubmitting(false);
    onClose();
  }
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-100">
            Schedule {config.shortName} Session
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-500">
            The session will unlock at the scheduled time and stay open for 1 hour.
          </p>
        </div>
