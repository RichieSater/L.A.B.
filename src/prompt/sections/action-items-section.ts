...
function getCompletionTiming(item: ActionItem): string {
  if (item.due === 'ongoing' || !item.completedDate) return '';
  const diff = daysBetween(item.due, item.completedDate);
...
export function buildActionItemsSection(state: AdvisorState): string {
  const openItems = state.actionItems.filter(item => item.status === 'open');
  const deferredItems = state.actionItems.filter(item => item.status === 'deferred');
  const completedItems = state.actionItems
    .filter(item => item.status === 'completed')
    .sort((a, b) => (b.completedDate ?? '').localeCompare(a.completedDate ?? ''));

  const totalOverdue = openItems.filter(
    item => item.due !== 'ongoing' && isOverdue(item.due),
  ).length;
  ...
  if (openItems.length === 0) {
    ...
    for (const item of openItems) {
      const overdue = item.due !== 'ongoing' && isOverdue(item.due);
      const overdueTag = overdue ? ` ** OVERDUE (${daysAgo(item.due)} days) **` : '';
      const sessionLabel = getSessionLabel(item.sourceSessionDate, state.sessions);
      section += `- [${item.id}] ${item.task} (due: ${item.due}, priority: ${item.priority}, assigned: ${sessionLabel})${overdueTag}\n`;
    }
  }
  ...
  if (deferredItems.length > 0) {
    ...
    for (const item of deferredItems) {
      ...
      section += `- [${item.id}] ${item.task} (new due: ${item.newDue ?? item.due}${reason}, assigned: ${sessionLabel})\n`;
    }
  }
